...
/** Empty cwd so tests do not inherit this workspace's site.json / public/og.jpg. */
function emptyOgCwd() {
  return mkdtempSync(join(tmpdir(), "grok-pwa-iso-"));
}
...
test("platform chrome overwrites share-card metas and always sets og:title", () => {
  ...
  const out = injectGrokPwaHead(html, { appName: "Wild Race", cwd: emptyOgCwd() });
...
test("streaming injector matches </HEAD> case-insensitively", () => {
  const injector = createHeadInjector({ appName: "Wild Race", cwd: emptyOgCwd() });
  const chunks = [
