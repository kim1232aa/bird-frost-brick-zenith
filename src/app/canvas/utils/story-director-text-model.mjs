...
function isProviderModelSelection(value) {
  return Boolean(
    value &&
      typeof value === "object" &&
      cleanText(value.providerId) &&
      cleanText(value.model),
  );
}

function normalizedProviderOptions(availableTextModels) {
  const options = [];
  const seen = new Set();
  for (const candidate of availableTextModels || []) {
    if (!candidate || typeof candidate !== "object") continue;
    const providerId = cleanText(candidate.providerId);
    ...
    options.push({
      providerId,
      model,
      value: cleanText(candidate.value) || key,
      label:
        cleanText(candidate.label) ||
        `${cleanText(candidate.providerName) || providerId} · ${model}`,
      providerName: cleanText(candidate.providerName) || providerId,
    });
  }
  return options;
}

function normalizedLegacyTextModels(availableTextModels) {
...
function usesProviderOptions(availableTextModels) {
...
function savedStoryDirectorTextModelSelection(metadata) {
...
function unresolvedOption(requested, status) {
...
export function resolveStoryDirectorTextModelPresentation(
  ...
  const unresolvedInherited =
    !inheritedOption && inheritedResolution.status !== "empty"
      ? unresolvedOption(inheritedTextModel, inheritedResolution.status)
      : undefined;
