            ...
            <div className="space-y-4">
                <div className="flex justify-center">
                    <div ref={boxRef} className="relative inline-block max-w-full overflow-hidden rounded-lg bg-black select-none">
                        {imageError ? (
                            <div role="alert" className="grid min-h-56 min-w-72 place-items-center px-5 text-center text-sm font-medium text-red-300">{imageError}</div>
                        ) : (
                            <>
                ...
                <div className="flex flex-wrap items-center justify-between gap-3 rounded-lg border px-3 py-2">
                    ...
                    <Button disabled={!image || Boolean(imageError)} icon={locked ? <Lock className="size-4" /> : <LockOpen className="size-4" />} onClick={() => setLocked((value) => !value)}>
                        {locked ? "锁定比例" : "自由比例"}
                    </Button>
                </div>

                <div className="flex items-center justify-end gap-2">
                    <Button disabled={!image || Boolean(imageError)} onClick={() => setCrop(defaultCrop)}>重置</Button>
                    <Button icon={<X className="size-4" />} onClick={onClose}>
                        取消
                    </Button>
                    <Button type="primary" icon={<Check className="size-4" />} disabled={!image || Boolean(imageError)} onClick={() => onConfirm(crop)}>
                        确认裁剪
                    </Button>
                </div>
            </div>
