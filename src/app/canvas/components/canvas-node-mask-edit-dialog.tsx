            ...
            <div className="grid gap-5 lg:grid-cols-[minmax(360px,1fr)_320px]">
                ...
                <div className="flex min-h-[360px] flex-col gap-5">
                    <div>
                        ...
                        <div className="mt-2 text-sm opacity-60">{imageError ? <span role="alert" className="font-medium text-red-500">{imageError}</span> : image ? `${image.width} x ${image.height}px` : "读取中"}</div>
                    </div>

                    <div className="grid grid-cols-2 gap-2">
                        <Button disabled={!image || Boolean(imageError)} type={mode === "paint" ? "primary" : "default"} icon={<Brush className="size-4" />} onClick={() => setMode("paint")}>
                            画笔
                        </Button>
                        <Button disabled={!image || Boolean(imageError)} type={mode === "erase" ? "primary" : "default"} icon={<Eraser className="size-4" />} onClick={() => setMode("erase")}>
                            擦除
                        </Button>
                    </div>
                    ...
                    <div className="space-y-2">
                        ...
                        <Slider disabled={!image || Boolean(imageError)} min={8} max={160} step={2} value={brushSize} onChange={setBrushSize} />
                    </div>

                    <div className="space-y-2">
                        <div className="text-sm font-medium opacity-75">修改要求</div>
                        <Input.TextArea
                            disabled={!image || Boolean(imageError)}
                            rows={6}
                            value={prompt}
                    ...
                    <div className="mt-auto flex items-center justify-between gap-2">
                        <Button disabled={!image || Boolean(imageError)} icon={<RotateCcw className="size-4" />} onClick={resetMask}>
                            重置
                        </Button>
                        <div className="flex items-center gap-2">
                            ...
                            <Button type="primary" icon={<WandSparkles className="size-4" />} disabled={!image || Boolean(imageError)} onClick={submit}>
                                AI 修改
                            </Button>
                        </div>
                    </div>
