...
import { Alert, App, Button, Empty, Input, Modal, Pagination, Spin, Tabs, Tag } from "antd";
...
import { createLibraryInsertPayload, libraryKindFilterOptions, type InsertAssetPayload } from "./asset-picker-contract";

export type AssetPickerTab = "my-assets" | "library";

export type { InsertAssetPayload } from "./asset-picker-contract";
...
function LibraryTab({ onInsert }: { onInsert: (payload: InsertAssetPayload) => void }) {
    ...
    const handleInsert = async (asset: AssetLibraryItem) => {
        try {
            ...
            if (asset.type === "text") {
                onInsert(createLibraryInsertPayload(asset));
            } else if (asset.type === "video") {
                onInsert(createLibraryInsertPayload(asset));
            } else {
                const dataUrl = await remoteImageToDataUrl(asset.url);
                onInsert(createLibraryInsertPayload(asset, dataUrl));
            }
        } catch {
            message.error("插入失败");
        } finally {
        ...
        <div className="space-y-4">
            <div className="flex flex-wrap items-center gap-3">
                ...
                <div className="flex gap-1.5">
                    {libraryKindFilterOptions.map((opt) => (
                        <Tag.CheckableTag
                            key={opt.value || "all"}
...
function MyAssetsTab({ onInsert }: { onInsert: (payload: InsertAssetPayload) => void }) {
    ...
    const hydrationStatus = useAssetStore((state) => state.hydrationStatus);
    const hydrationError = useAssetStore((state) => state.hydrationError);
    const retryHydration = useAssetStore((state) => state.retryHydration);
    const [keyword, setKeyword] = useState("");
    ...
    return (
        <div className="space-y-4">
            ...
            {hydrationStatus === "loading" ? (
                <div className="flex flex-col items-center justify-center gap-3 py-16" role="status" aria-label="正在读取我的素材">
                    <Spin />
                    <span className="text-sm text-stone-500 dark:text-stone-400">正在读取已保存的素材…</span>
                </div>
                ...
                <Alert
                    ...
                    showIcon
                    message="我的素材读取失败"
                    description={hydrationError || "未显示空素材列表，也没有写入默认数据。请重试读取。"}
                    action={<Button size="small" onClick={() => void retryHydration()}>重试读取</Button>}
                />
            ) : visible.length ? (
                <div className="grid grid-cols-4 gap-3">
                    {visible.map((asset) => (
