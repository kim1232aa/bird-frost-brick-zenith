...
import { Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Pencil, Pin, PinOff, Plus, RefreshCw, Settings2, Trash2, Upload, Video } from "lucide-react";
...
type CanvasNodeHoverToolbarProps = {
    ...
    onToggleRetain: (node: CanvasNodeData) => void;
    onLayerEdit: (node: CanvasNodeData) => void;
...
export function CanvasNodeHoverToolbar({
    ...
    if (HIDE_CANVAS_NODE_HOVER_TOOLBAR) return null;
    if (!node) return null;
    ...
    const imageTools = buildImageToolbarTools(node, { onUpload, onToggleFreeResize, onLayerEdit, onMaskEdit, onCrop, onSplit, onUpscale, onSuperResolve, onAngle, onViewImage, onCopyPrompt: copyImagePrompt, onReversePrompt }).filter((tool) => tool.id !== "superResolve");
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...
        ...(hasImage ? [{ id: "retain", title: node.metadata?.retained ? "取消长期保留" : "长期保留图片", label: node.metadata?.retained ? "取消保留" : "保留", icon: node.metadata?.retained ? <PinOff className="size-4" /> : <Pin className="size-4" />, active: Boolean(node.metadata?.retained), onClick: () => onToggleRetain(node) }] : []),
                        ...
                        <div className="thin-scrollbar h-full space-y-3 overflow-auto pr-1">
                            ...
                            {node.metadata?.errorDetails ? (
                                <div className="rounded-lg border p-3 text-red-400" style={{ borderColor: theme.node.stroke }}>
                                    {formatCanvasGenerationError(node.metadata.errorDetails)}
                                </div>
                            ) : null}
                        </div>
                    ) : (
