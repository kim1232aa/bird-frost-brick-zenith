import type { ResolvedVideoModelCapability } from "./video-model-capabilities";

export type VideoPromptRequirement = {
  kind: "required" | "required-without-media";
  reason: string;
};

export type VideoPromptMaterialSummary = {
  imageCount?: number;
  videoCount?: number;
  audioCount?: number;
};

/** Exact prompt rules with current first-party evidence. Unknown profiles only
 * receive the universal no-prompt-and-no-media guard; they are not guessed to
 * require text when an image/video/audio input is present. */
export function resolveVideoPromptRequirement(
  capability: Pick<
    ResolvedVideoModelCapability,
    "id" | "provider" | "providerLabel" | "model"
  >,
): VideoPromptRequirement {
...
export function videoPromptPreflightError(
  capability: Pick<
    ResolvedVideoModelCapability,
    "id" | "provider" | "providerLabel" | "model"
  >,
  prompt: string,
  material: VideoPromptMaterialSummary = {},
) {
