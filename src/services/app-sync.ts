...
import { downloadWebdavFile, MAX_WEBDAV_FILE_BYTES, MAX_WEBDAV_MANIFEST_BYTES, uploadWebdavFile, WEBDAV_MANIFEST_FILE_NAME } from "@/services/webdav-sync";
import { mergeSyncRecords, mergeSyncTombstones, type SyncTombstone } from "@/services/sync-record-merge";
import type { Asset } from "@/stores/use-asset-store";
import { useAssetStore } from "@/stores/use-asset-store";
import type { WebdavSyncConfig } from "@/stores/use-config-store";
import type { CanvasProject } from "@/app/canvas/stores/use-canvas-store";
...
type SyncDomainOptions<T> = {
    ...
    emptyData: T;
    mergeData: (local: T, remote: T, deleted: SyncTombstone[]) => { data: T; deleted: SyncTombstone[] };
    applyData?: (data: T, deleted: SyncTombstone[]) => Promise<void>;
};
...
const imageLogStore = {
    stateKey: "logs:image_generation_logs",
    legacy: localforage.createInstance({ name: "infinite-canvas", storeName: "image_generation_logs" }),
};
const videoLogStore = {
    stateKey: "logs:video_generation_logs",
    legacy: localforage.createInstance({ name: "infinite-canvas", storeName: "video_generation_logs" }),
};
...
export async function syncAppDataToWebdav(config: WebdavSyncConfig, onProgress?: AppSyncProgress): Promise<AppSyncResult> {
    ...
    const [canvas, assets, imageLogs, videoLogs] = await Promise.all([
        syncDomain<CanvasDomainData>(config, onProgress, {
            ...
            localDeleted: () => useCanvasStore.getState().syncDeleted,
            mergeData: (local, remote, deleted) => {
                const merged = mergeSyncRecords({ local: local.projects, remote: remote.projects, remoteDeleted: deleted, timeKey: "updatedAt" });
                return { data: { projects: merged.records }, deleted: merged.deleted };
            },
            applyData: async (data, deleted) => useCanvasStore.getState().replaceProjects(data.projects, deleted),
        }),
        syncDomain<AssetDomainData>(config, onProgress, {
            ...
            localDeleted: () => useAssetStore.getState().syncDeleted,
            mergeData: (local, remote, deleted) => {
                const merged = mergeSyncRecords({ local: local.assets, remote: remote.assets, remoteDeleted: deleted, timeKey: "updatedAt" });
                return { data: { assets: merged.records }, deleted: merged.deleted };
            },
            applyData: async (data, deleted) => useAssetStore.getState().replaceAssets(await Promise.all(data.assets.map(hydrateAsset)), deleted),
        }),
        syncDomain<LogDomainData>(config, onProgress, {
            ...
            mergeData: (local, remote, deleted) => {
                const merged = mergeSyncRecords({ local: local.logs, remote: remote.logs, remoteDeleted: deleted, timeKey: "createdAt" });
                return { data: { logs: merged.records }, deleted: merged.deleted };
            },
...
async function syncDomain<T>(config: WebdavSyncConfig, onProgress: AppSyncProgress | undefined, options: SyncDomainOptions<T>): Promise<SyncDomainResult<T>> {
    try {
        ...
        const deleted = mergeSyncTombstones(remoteManifest?.deleted || [], options.localDeleted?.() || []);
        const merged = remoteManifest
            ? options.mergeData(localData, remoteManifest.data, deleted)
            : { data: localData, deleted };
        ...
        if (remoteManifest) {
            ...
            await options.applyData?.(mergedData, merged.deleted);
        }
        ...
        const manifest: DomainManifest<T> = { app: "infinite-canvas", version: 2, domain: options.key, exportedAt: new Date().toISOString(), data: mergedData, files: uploaded.files, deleted: merged.deleted };
...
async function readDomainManifest<T>(config: WebdavSyncConfig, domain: DomainKey, emptyData: T): Promise<DomainManifest<T> | null> {
    const file = await downloadWebdavFile(config, domainPath(domain, WEBDAV_MANIFEST_FILE_NAME), { maxBytes: MAX_WEBDAV_MANIFEST_BYTES });
    if (!file) return null;
...
async function downloadMissingFiles<T>(config: WebdavSyncConfig, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    await runWithConcurrency(tasks, FILE_CONCURRENCY, async (remoteFile) => {
        const expectedBytes = Number(remoteFile.bytes);
        if (!Number.isSafeInteger(expectedBytes) || expectedBytes <= 0 || expectedBytes > MAX_WEBDAV_FILE_BYTES) {
            throw new Error(`WebDAV 清单中的文件大小无效：${remoteFile.path}`);
        }
        const blob = await downloadWebdavFile(config, remoteFile.path, { maxBytes: expectedBytes, expectedBytes });
        if (!blob) return;
...
async function uploadChangedFiles<T>(config: WebdavSyncConfig, domain: DomainKey, data: T, remoteFiles: AppSyncFile[], onProgress?: AppSyncProgress) {
    ...
    for (const storageKey of storageKeys) {
        ...
        files.push(item);
        if (!remoteFile || remoteFile.bytes !== blob.size || !isValidSha256(remoteFile.sha256) || remoteFile.sha256.toLowerCase() !== sha256) {
...
async function readStoredLogs(store: LogStore) {
    try {
        const current = await getDesktopJSON<StoredLog[]>(store.stateKey);
        if (Array.isArray(current)) return current;
        const legacyLogs = await readLegacyLogs(store);
        if (legacyLogs.length) {
            await setDesktopJSON(store.stateKey, legacyLogs);
            await store.legacy.clear();
        }
        return legacyLogs;
    } catch {
        return readLegacyLogs(store);
    }
}

async function readLegacyLogs(store: LogStore) {
    const logs: StoredLog[] = [];
    await store.legacy.iterate<StoredLog, void>((value) => {
