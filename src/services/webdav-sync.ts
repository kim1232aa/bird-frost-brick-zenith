...
import { readBoundedResponseBlob } from "@/services/bounded-response-blob";

import type { WebdavSyncConfig } from "@/stores/use-config-store";

export const WEBDAV_MANIFEST_FILE_NAME = "manifest.json";
export const MAX_WEBDAV_MANIFEST_BYTES = 16 * 1024 * 1024;
export const MAX_WEBDAV_FILE_BYTES = 2 * 1024 * 1024 * 1024;
...
export async function downloadWebdavSyncFile(config: WebdavSyncConfig) {
    return downloadWebdavFile(config, WEBDAV_MANIFEST_FILE_NAME, { maxBytes: MAX_WEBDAV_MANIFEST_BYTES });
}
...
export async function downloadWebdavFile(
    ...
    await ensureWebdavDirectory(config);
    return webdavFetch(config, path, { method: "GET" }, async (response, controller) => {
        if (response.status === 404) return null;
        if (!response.ok) await throwWebdavError(response, "读取 WebDAV 同步文件失败");
        const maxBytes = Math.min(MAX_WEBDAV_FILE_BYTES, Math.max(1, options.maxBytes || MAX_WEBDAV_FILE_BYTES));
        const file = await readBoundedResponseBlob(response, maxBytes, () => controller.abort());
        if (options.expectedBytes !== undefined && file.size !== options.expectedBytes) {
            throw new Error(`WebDAV 文件大小校验失败：预期 ${options.expectedBytes} 字节，实际 ${file.size} 字节`);
        }
...
async function webdavFetch(config: WebdavSyncConfig, path: string, init: RequestInit): Promise<Response>;
async function webdavFetch<T>(config: WebdavSyncConfig, path: string, init: RequestInit, consume: (response: Response, controller: AbortController) => Promise<T>): Promise<T>;
async function webdavFetch<T>(config: WebdavSyncConfig, path: string, init: RequestInit, consume?: (response: Response, controller: AbortController) => Promise<T>) {
    ...
    try {
        ...
        const response = config.proxyMode === "nextjs"
            ? await desktopFetch(desktopApiUrl("/webdav-proxy"), { method: "POST", headers: proxyHeaders(url, init.method || "GET", headers), body: proxyBody(init), signal: controller.signal })
            : await fetch(url, { ...init, headers, signal: controller.signal });
        return consume ? await consume(response, controller) : response;
    } catch (error) {
