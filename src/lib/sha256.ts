/** SHA-256 hex. WebCrypto when present; deterministic fallback for insecure HTTP (no subtle). */
...
export async function sha256BytesHex(bytes: Uint8Array<ArrayBuffer>): Promise<string> {
  const digestFn = globalThis.crypto?.subtle?.digest?.bind(globalThis.crypto.subtle);
  if (digestFn) {
    const digest = await digestFn("SHA-256", bytes);
